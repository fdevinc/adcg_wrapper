/******************************************************************************
 *
 * @file adcg_wrapper/autodiff/function.hpp
...
#ifndef _ADCG_WRAPPER__AUTODIFF__FUNCTION_HPP_
#define _ADCG_WRAPPER__AUTODIFF__FUNCTION_HPP_

#include <finitediff/finitediff.hpp>

#include "adcg_wrapper/autodiff/data_types.hpp"
#include "adcg_wrapper/io/logging.hpp"
#include "adcg_wrapper/utils/passkey.hpp"
#include "adcg_wrapper/utils/utils.hpp"
...
class Function {  // clang-format on
  public:
    struct Blueprint {
        Blueprint(const ADFunction& functionImpl_,
                  ...
                  const std::filesystem::path& folder_         = ADCG_WRAPPER_CODEGEN_FOLDER)
    ...
    Function(Passkey<FunctionFactory>,
        ...
        if (_model->isHessianSparsityAvailable()) {
            ADCG_WRAPPER_ASSERT_MSG(_dependentVariableSize == 1_idx,
    ...
    void Evaluate(const Eigen::MatrixBase<_XP>& xp, Eigen::MatrixBase<_Y> const& y) const {  // clang-format on
        ADCG_WRAPPER_ASSERT(xp.size() == _independentVariableSize + _parameterSize);
        ADCG_WRAPPER_ASSERT(y.size() == _dependentVariableSize);
    ...
    template <typename _XP,
              ADCGWrapper::Concepts::ContiguousRangeOf<real_t> _Y>  // clang-format off
    requires std::same_as<typename _XP::Scalar, real_t>
    void Evaluate(const Eigen::MatrixBase<_XP>& xp, _Y&& y) const {  // clang-format on
        ADCG_WRAPPER_ASSERT(xp.size() == _independentVariableSize + _parameterSize);
        ADCG_WRAPPER_ASSERT(static_cast<index_t>(std::ranges::size(std::forward<_Y>(y))) ==
                            _dependentVariableSize);
    ...
    const Eigen::Map<const Eigen::SparseMatrix<real_t, Eigen::RowMajor>>& Jacobian(
        ...
        ADCG_WRAPPER_ASSERT(ImplementsJacobian());
        ADCG_WRAPPER_ASSERT(xp.size() == _independentVariableSize + _parameterSize);
    ...
    const Eigen::Map<const Eigen::SparseMatrix<real_t, Eigen::RowMajor>>& Hessian(
        ...
        ADCG_WRAPPER_ASSERT(ImplementsHessian());
        ADCG_WRAPPER_ASSERT_MSG(_dependentVariableSize == 1_idx,
                                "Although this method is meant for general multivariable "
                                "functions, it is "
                                "currently implemented only for scalar functions.");
        ADCG_WRAPPER_ASSERT(dependentVariableIndex >= 0_idx &&
    ...
    const Eigen::Map<const Eigen::SparseMatrix<real_t, Eigen::RowMajor>>& Hessian(
        const Eigen::MatrixBase<_XP>& xp) const {
        ADCG_WRAPPER_ASSERT(ImplementsHessian());
        ADCG_WRAPPER_ASSERT(_dependentVariableSize == 1_idx);
        ADCG_WRAPPER_ASSERT(xp.size() == _independentVariableSize + _parameterSize);
    ...
    [[nodiscard]] bool TestFunction(
        ...
        ADCG_WRAPPER_ASSERT(ImplementsFunction());
  ...
  private:
    ...
    /// @note For both the Jacobian and the Hessian, we store maps to arrays of
    /// data
    ///       that are manipulated by the \c Autodiff::Function generated code.
    ///       Unfortunately, \a CppADCodeGen generates sparse matrices in row
    ///       major representation with outer indices sorted in decreasing order,
    ///       while \a Eigen sorts them in increasing order. This means that the
    ///       maps we create cannot be used reliably for sparse operations.
    ///       However, assigning them to column major sparse matrices results in
    ...
    class Worker {
      ...
      private:
        ...
        void CreateModelsImpl(const std::string& modelName,
            ...
            ADCG_WRAPPER_ASSERT(y.size() && y.size() == _blueprint.dependentVariableSize);
            ...
            ADCG_WRAPPER_LOG(info, "Compiling shared library {}...", tmpLibrary.c_str());
            _dynamicLib = libraryProcessor.createDynamicLibrary(gccCompiler);
            _model      = _dynamicLib->model(modelName);
            ADCG_WRAPPER_ASSERT(_dynamicLib);
            ADCG_WRAPPER_ASSERT(_model);

            ADCG_WRAPPER_LOG(info, "Renaming {} to {}...", tmpLibrary.c_str(), library.c_str());
            std::filesystem::rename(tmpLibrary, library);
        }

        void LoadModel() {
            ADCG_WRAPPER_LOG(info, "Loading shared library {}...", _library.c_str());
...
#endif /* _ADCG_WRAPPER__AUTODIFF__FUNCTION_HPP_ */
